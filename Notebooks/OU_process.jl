...
# ╔═╡ 6ed3e45e-c309-11ed-2f82-e7f30e8f7afa
begin
...
# ╔═╡ 6403a358-b6de-4f0f-9b03-166d4aa7b6d1
begin
	# --> Parameters of the harmonic oscillator.
	p = ξ, σ_process, σ_sensor = 0.5, 0.2, 0
	...
	G = exp(F)
	A = exp(A*Δt)
	Q = Hermitian(A * G[1:2, 3:end])
	N_process = MvNormal(zeros(2), Q)
	N_sensor = Normal(0, σ_sensor)
end

# ╔═╡ 869b4d2b-3ae4-480e-a1bb-969f7e745759
eigvals(A)

# ╔═╡ fe4327c5-491b-4f93-815a-de0ec486dfc4
begin
	x = rand(MvNormal(zeros(size(A, 1)), lyapd(A, Q)))
	y = Array[]

	t = 0:Δt:100_000

	for _ in t
		push!(y, C*x)# .+ rand(N_sensor))
		x = A*x + rand(N_process)
	end
...
# ╔═╡ 3255a19e-3aaf-405d-9452-d862e17bcbb5
let
	size_inches = (9, 3)
	...
	ax = Axis(
		...
		title="Orstein-Uhlenbeck process",
		titlealign=:left,
		subtitle="Typical time-series of the Orstein-Uhlenbeck process with a = -1 and b = 0.2.",
		subtitlecolor=:gray,
		xgridvisible=false,
		ygridvisible=false,
	)
	lines!(t[1:2000], y[1:2000])
	xlims!(0, t[2000])
	hlines!(0, linestyle=:dash, color=:black)
	hidespines!(ax, :t, :r)
	save("OU_process.png", f, px_per_unit=10)

	f
end

# ╔═╡ 798d7ab4-46bf-4287-acce-69514d23c8d2


# ╔═╡ 33cff04d-127f-4481-afcf-d76d6661596b


# ╔═╡ 40be7264-c0fa-499c-a631-94d7001eaf74


# ╔═╡ 1c0eebaa-3775-4fe4-86c3-9e084f177130


# ╔═╡ 4c0ac3cf-aba4-4be3-a334-c70f42afdfbd


# ╔═╡ ceeed647-f7a8-4f1f-849f-1004c2b7e762


# ╔═╡ 36f1d088-bf17-4e35-a062-337a60a65439
let
	P = lyapd(A, Q)
	R = [(C * A^k * P * C')[1] for k = 1:100000]
	pushfirst!(R, (C * P * C' + D * σ_sensor^2 * D')[1])
	lines(R)
end

# ╔═╡ 59c87bf8-3e27-4a8b-b4a1-05996c7cbe69


# ╔═╡ ff1bb126-e090-46e3-8b84-56d50b93aa13


# ╔═╡ e198ea34-5eaf-4bfb-86c4-584494339cb8


# ╔═╡ e658c4b7-f3be-4d60-895d-0306daceb03c


# ╔═╡ 1b52943a-ec1c-44c7-9d0d-655200b6b604


# ╔═╡ 811178d3-fdf0-4483-93fd-52a9bbb6fdef


# ╔═╡ 6ecae6e5-f905-4e4f-b4c9-11ee98b09f05


# ╔═╡ 66ece582-896d-4b94-ac88-6fed3bc4e67e


# ╔═╡ 3a27721c-a90a-4c19-9d41-bdbadc5a1362


# ╔═╡ 56763e72-31f2-4bfd-96b4-0e4c7dd2ce1a


# ╔═╡ 8359ac0b-eb79-49f4-90e3-1fd8a86cbb9c


# ╔═╡ 7b983427-bfb4-422a-a669-15e14705b9b4


# ╔═╡ ce9c2640-92e9-4156-87eb-e4d70b6d7564


# ╔═╡ de89e4d3-f99e-45a0-8f05-9cd0b5beb3e1


# ╔═╡ 90c82e08-ee30-4b1a-9e83-e120d298e83b


# ╔═╡ 48bd0f73-51e3-4339-a564-ff05c03c5fc7


# ╔═╡ 0ff83e4a-62cd-489e-955f-648c1d731d97


# ╔═╡ 9fee4ba3-f702-4dd9-9908-dcbecb9cca61


# ╔═╡ 61209f29-f465-4771-a948-1bcf3bb7df21


# ╔═╡ 756a1d03-298a-4d41-b9c7-4bf04984bd47
size(y)

# ╔═╡ dfbfe7c4-3bfe-4b6f-80c6-759c375b885c
let
	m = 3
	k = 1
	y = measurements
	n = length(y) - k
	# --> Basis functions.
	# y ./= 0.5(maximum(y) - minimum(y))
	X = hcat([y[1:n].^p for p = 1:m]...)'
	Y = hcat([y[1+k:n+k].^p for p = 1:m]...)'
	...
	rank(σ) = findfirst( cumsum(σ.^2) / sum(σ.^2) .> 0.999)
	# r = rank(S₀)
	...
	@show eigvals(U * Diagonal(K) * V')
	@show svdvals(U * Diagonal(K) * V')

	# --> Singular functions in orginal basis.
	F = zeros(size(U, 1), size(U, 1))
	F[1, 1] = 1
	F[2:end, 2:end] = Q₁ * Diagonal( .√S₁ ) * Q₁'
	...
	Vb = F * V
	YY = hcat([ones(size(X, 2)), (X)']...)'
	XX = hcat([ones(size(Y, 2)), (Y)']...)'
	...
	ax = Axis(
		...
		title="Orstein-Uhlenbeck process",
		titlealign=:left,
		subtitle="Leading Koopman eigenfunctions consist\nin simple polynomials in x.",
		subtitlecolor=:gray,
		xgridvisible=false,
		ygridvisible=false,
	)
	...
	coef = Ub[:, 1]
	g(x) = coef[1] + coef[2]*(x-π₀[1]) + coef[3]*(x^2-π₀[2]) + coef[4]*(x^3-π₀[3])
	z = g.(x)
	z ./= maximum(abs.(z))
	lines!(ax, x, z, linewidth=2, label=L"\varphi_1")

	coef = Ub[:, 2]
	g(x) = coef[1] + coef[2]*(x-π₀[1]) + coef[3]*(x^2-π₀[2]) + coef[4]*(x^3-π₀[3])
	z = g.(x)
	z ./= maximum(abs.(z))
	lines!(ax, x, z, linewidth=2, label=L"\varphi_2")

	coef = Ub[:, 3]
	g(x) = coef[1] + coef[2]*(x-π₀[1]) + coef[3]*(x^2-π₀[2]) + coef[4]*(x^3-π₀[3])
	z = g.(x)
	z ./= maximum(abs.(z))
	lines!(ax, x, z, linewidth=2, label=L"\varphi_3")

	coef = Ub[:, 4]
	g(x) = coef[1] + coef[2]*(x-π₀[1]) + coef[3]*(x^2-π₀[2]) + coef[4]*(x^3-π₀[3])
	z = g.(x)
	z ./= maximum(abs.(z))
	lines!(ax, x, z, linewidth=2, label=L"\varphi_4")
	
	ylims!(ax, -1.1, 1.1)
	xlims!(ax, -1, 1)
	axislegend(ax, position=:ct, framevisible=false, nbanks=2)
	save("OU_process_eigenfunctions.png", f, px_per_unit=10)
	f
end

# ╔═╡ 36bf6811-d6e2-45a5-9e0c-1ef17d4ded8b


# ╔═╡ 9685b5cc-50bc-48a2-8e0d-c66156105ca3
...
# ╔═╡ be06acd7-69d6-4771-98de-44c5336df85a
...
# ╔═╡ deced902-5ef5-40bf-b60f-b3c340d35b71
function TCCA(X, Y)
...
# ╔═╡ ec17609d-251e-4f3a-ba6e-6079d957306e
let
	M = 1:10
	k = 1
	...
	for m in M
		...
		K, U, V = TCCA(X, Y)
		# K, U, V = K[1:r], U[:, 1:r], V[:, 1:r]
		push!(vamp_score_train, ℛₑ(K, U, V, C₀₀, C₁₁, C₁₀))
		score_1 = tr(2*Diagonal(K) * U' * C₁₀ * V)
		score_2 = tr(Diagonal(K) * U' * C₁₁ * U * Diagonal(K) * V' * C₀₀ * V)
		@show score_1, score_2
		...
		for _ = 1:10
			...
			for _ in t
				push!(y_test, C*x)# .+ rand(N_sensor))
				x = A*x + rand(N_process)
			end
		
			y_test = hcat(y_test...)
			y_test = reshape(y_test, length(y_test))

			# y ./= 0.5(maximum(y) - minimum(y))
			X = hcat([y_test[1:n].^p for p = 1:m]...)'
			Y = hcat([y_test[1+k:n+k].^p for p = 1:m]...)'
	...
	save("OU_process_cross_validation.png", f, px_per_unit=10)
	f
end

# ╔═╡ 6367bd4a-85ed-42fc-a26f-7e8cdcb1212f


# ╔═╡ 1158f417-9f25-494e-9898-979b801e457d
let
	m = 3
	k = 1

	y = measurements
	N = collect(LinRange(10, 5000, 25))
	N = ceil.(Int, N)
	...
	ax = Axis(
		f[1, 1],
		ylabel=L"\mathcal{R}_e(n)",
		xlabel="Number of samples × 1000",
		# xticksvisible=false,
		# xticklabelsvisible=false,
		# yticksvisible=false,
		# yticklabelsvisible=false,
		...
		titlealign=:left,
		subtitle="Evolution of the VAMP-E ℛₑ score as a\nfunction of the number of samples.",
		subtitlecolor=:gray,
		xgridvisible=false,
		ygridvisible=false,
	)

	lines!(N/1000, vamp_score_train, label="Train", color=:black, linewidth=2)
	lines!(N/1000, vamp_score_test, linestyle=:dash, label="Test", color=:red, linewidth=2)
	axislegend(ax, position=:rb, framevisible=false)
	# ax.xticks = 0:2:10
	xlims!(0, maximum(N / 1000))
	ylims!(0, 5)
	
	save("OU_process_learning_curve.png", f, px_per_unit=10)
	f
end
...
FFTW = "~1.6.0"
LaTeXStrings = "~1.3.0"
MatrixEquations = "~2.2.8"
SpecialMatrices = "~3.0.0"
StaticArrays = "~1.5.17"
StatsBase = "~0.33.21"
ToeplitzMatrices = "~0.8.0"
"""
...
julia_version = "1.8.4"
manifest_format = "2.0"
project_hash = "c4d82aa625433d69f4b1f3e4911bd6eb4ac9f695"
...
git-tree-sha1 = "a0b1a3217c1268389f91d58f6f020e01944a118d"
...
git-tree-sha1 = "5a3eae2c89fbe7c91ca596b961f65bd106167504"
